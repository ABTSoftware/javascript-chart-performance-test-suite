...
function eLinePerformanceTest(seriesNum, pointsNum) {
    ...
    const generateData = () => {
        ...
        DATA = { xValuesArrArr, yValuesArrArr, colorValuesArr };
    };

    const appendData = () => {
        const { xValuesArrArr, yValuesArrArr, colorValuesArr } = DATA;
            ...
            chart
                .addLineSeries({
                    schema: {
                        ...
                        yValues: { pattern: null, storage: Float64Array }
                    }
                })
                ...
                .setDataMapping({ x: 'xValues', y: 'yValues' })
                .appendSamples({
                    xValues: xValuesArrArr[i],
                    yValues: yValuesArrArr[i]
                });
        }
    ...
    const updateChart = (_frame) => {
        ...
        return seriesNum * DATA.xValuesArrArr[0].length;
    };
...
function eScatterPerformanceTest(seriesNum, pointsNum, divId = 'chart-root') {
    ...
    const appendData = () => {
        ...
        scatterSeries.setDataMapping({ x: 'xValues', y: 'yValues' })
        scatterSeries.appendSamples({
            xValues: xValuesArr,
            yValues: yValuesArr
        });
    };
    ...
    const updateChart = () => {
        ...
        return scatterSeries.getSampleCount();
    };
...
function eCandlestickPerformanceTest(seriesNum, pointsNum) {
    const { lightningChart, emptyFill, OHLCSeriesTypes, disableThemeEffects, Themes } = lcjs;
    ...
    const appendData = () => {
        ...
        const series = chart
            .addOHLCSeries({ type: OHLCSeriesTypes.Candlesticks })
            .setCursorEnabled(false)
            .setPackData(false)
            .setFigureWidth(0.8);
...
function eFifoEcgPerformanceTest(seriesNum, pointsNum, incrementPoints) {
    ...
    const createChart = async () => {
        chart = lightningChart(license)
            ...
            .setTitleFillStyle(emptyFill)
            .setPadding({ top: 10, bottom: 10, left: 10, right: 10 });
        const xAxis = chart.getDefaultAxisX().setAnimationScroll(undefined).setScrollStrategy(AxisScrollStrategies.fitting);
        const yAxis = chart.getDefaultAxisY().setAnimationScroll(undefined).setScrollStrategy(AxisScrollStrategies.fitting);
    };
...
function eMountainPerformanceTest(seriesNum, pointsNum, divId = 'chart-root') {
    ...
    const appendData = () => {
        const { xValuesArr, yValuesArr } = DATA;
        const series = chart.addAreaSeries().setCursorEnabled(false);
        series.appendSamples({
            xValues: xValuesArr,
            yValues: yValuesArr
        });
        // Calculate min/max directly from data — yAxis.fit() is deferred in v8
        // so reading getInterval() immediately after would return the default range.
        let dataMin = Infinity, dataMax = -Infinity;
        for (let i = 0; i < yValuesArr.length; i++) {
            if (yValuesArr[i] < dataMin) dataMin = yValuesArr[i];
            if (yValuesArr[i] > dataMax) dataMax = yValuesArr[i];
        }
        yAxis = chart.getDefaultAxisY();
        yAxis.setInterval({ start: dataMin, end: dataMax });
        const maxVal = Math.max(Math.abs(dataMin), Math.abs(dataMax));
        delta = maxVal / 300;
    };

    const updateChart = (_frame) => {
        yAxis.setInterval({ start: yAxis.getInterval().start - delta, end: yAxis.getInterval().end + delta });
        return seriesNum * DATA.xValuesArr.length;
    };
...
function eSeriesCompressionPerformanceTest(seriesNum, pointsNum, incrementPoints, divId = 'chart-root') {
    const {lightningChart, emptyFill, SolidLine, SolidFill, ColorHEX, disableThemeEffects, Themes, AxisScrollStrategies } = lcjs;
    ...
    const createChart = async () => {
        ...
        chart = lightningChart(license)
          ...
          .setPadding({top: 10, bottom: 10, left: 10, right: 10})
          .setTitleFillStyle(emptyFill);
        chart.getDefaultAxisX().setAnimationScroll(undefined).setScrollStrategy(AxisScrollStrategies.fitting);
        yAxis = chart.getDefaultAxisY().setAnimationScroll(undefined).setScrollStrategy(AxisScrollStrategies.fitting);
    };
    ...
    const appendData = () => {
        ...
        series = chart
            ...
            .setStrokeStyle(new SolidLine({thickness: 2, fillStyle: new SolidFill({color: ColorHEX('#00FF00')})}));
        series.setDataMapping({x: 'xValues', y: 'yValues'});
        series.appendSamples({ xValues: xValuesArr, yValues: yValuesArr });
    };
...
function eMultiChartPerformanceTest(seriesNum, pointsNum, incrementPoints, chartsNum) {
    ...
    const createChart = async () => {
        ...
        for (let c = 0; c < chartsNum; c++) {
            ...
            handlers.push(h);
            await h.createChart();
            // LCJS v8 overwrites container div positioning on chart creation — restore it
            const slotDiv = document.getElementById(slotDivId);
            slotDiv.style.position = 'absolute';
            slotDiv.style.width = `${chartWidth}%`;
            slotDiv.style.height = `${chartHeight}%`;
            slotDiv.style.left = `${(c % cols) * chartWidth}%`;
            slotDiv.style.top = `${Math.floor(c / cols) * chartHeight}%`;
        }
    };
...
function e3dPointCloudPerformanceTest(seriesNum, pointsNum) {
    ...
    const createChart = async () => {
        chart3D = lightningChart(license)
            .Chart3D({
                container: document.getElementById('chart-root'),
                disableAnimations: true,
                theme: disableThemeEffects(Themes.darkGold),
